...
def _process_chunk(pdf_paths, pdf_backend, output_dir, num_threads, metadata_lookup, debug_data_path, parser_type):
...
def _process_chunk_mineru(pdf_paths, output_dir, num_threads, metadata_lookup, debug_data_path):
    """Helper function to process a chunk of PDFs using minerU."""
    parser = PDFParser(
        output_dir=output_dir,
        num_threads=num_threads,
    ...
    success_count, failure_count = parser._process_with_mineru(pdf_paths)
    return f"Processed {len(pdf_paths)} PDFs (success: {success_count}, failed: {failure_count})."
...
class PDFParser:
    def __init__(
        ...
        self.doc_converter = self._create_document_converter() if parser_type == ParserType.DOCLING else None
    ...
    def _process_with_mineru(self, pdf_paths: List[Path]) -> tuple[int, int]:
        """使用minerU处理PDF文件"""
        ...
        for pdf_path in pdf_paths:
            try:
                ...
                _log.error(f"Failed to process {pdf_path} with minerU: {str(e)}")
                failure_count += 1
    ...
    def parse_and_export(self, input_doc_paths: List[Path] = None, doc_dir: Path = None):
        ...
        # 确保使用实例的 parser_type 属性决定使用哪种解析方式
        if self.parser_type == ParserType.DOCLING:
            ...
            success_count, failure_count = self._process_with_mineru(input_doc_paths)
    ...
    def parse_and_export_parallel(
        ...
        # 根据解析器类型选择处理函数
        if self.parser_type == ParserType.MINERU:
            process_func = _process_chunk_mineru
            # 对于minerU，不需要pdf_backend参数
            args_list = [
                (chunk, self.output_dir, self.num_threads, 
                 self.metadata_lookup, self.debug_data_path)
                for chunk in chunks
            ]
        else:
            process_func = _process_chunk
            # 对于docling，需要pdf_backend参数
            args_list = [
                (chunk, self.pdf_backend, self.output_dir, 
                 self.num_threads, self.metadata_lookup, 
                 self.debug_data_path, self.parser_type)
                for chunk in chunks
            ]
...
class JsonReportProcessor:
    ...
    def assemble_tables(self, tables, data):
        ...
        for i, table in enumerate(tables):
            ...
            table_html = table.export_to_html(doc=data)     # 不带doc参数的方法已被弃用
